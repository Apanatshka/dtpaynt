...
# argument 6: timeout
# argument 7: name for the docker container
...
# NOTE: DTNEST does not actually use the timout value
DTNEST="python3 /opt/paynt/paynt.py /opt/paynt/models/dts-uai/ --sketch $MODELNAME --props $PROPNAME  --timeout $6 --add-dont-care-action"
...
docker run --name $7 --rm --volume="$1:/opt/paynt/models/dts-uai/$MODELNAME:ro" --volume="$2:/opt/paynt/models/dts-uai/$PROPNAME:ro" --volume="$3:/opt/paynt/models/dts-uai/state-valuations.json:ro" --volume="$4:/opt/paynt/models/dts-uai/scheduler.storm.json:ro" --volume="$5:/opt/paynt/models/dts-uai/results" paynt:uai sh -c "$SET_OUTPUT_NAME && $DTCONTROL && $DTNEST"
